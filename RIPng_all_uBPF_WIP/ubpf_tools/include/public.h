...
#define CALL_PRE_POST(plug_id, plug_args, nargs, ok_ret_val, ...) {\
...
#define CUST_CALL_ALL(plug_id, plug_args, nargs, args_vm_check, default_ret_val, on_err, post, ...){\
  uint64_t VM_RETURN_VALUE = 0; \
  bpf_full_args_t FULL_ARGS, *PTR_FULL_ARGS;\
  unsigned int PLUGIN_ID = plug_id;\
  int ___ubpf_the_err___ = 0; \
  ...
  if (!___ubpf_status___) ___ubpf_the_err___ = 1;\
    ...
    {post} \
    if (!args_vm_check(VM_RETURN_VALUE)) ___ubpf_the_err___ = 1;\
  }\
  if (___ubpf_the_err___) {\
    {on_err} \
  } else {\
