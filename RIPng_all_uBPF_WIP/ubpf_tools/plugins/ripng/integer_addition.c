...
static __attribute__((always_inline)) char* to_str(algebra* this, metric m, int metric)
{
    ...
    ubpf_sprintf(str, size_of_str(this, m, metric)+1, "%i", *(int*) m);
    return str;
}

ALWAYS_INLINE algebra* ADD(int n, int m, struct mal* init, struct mal** last)
{
    algebra* a = (algebra*) ctx_malloc(sizeof(algebra) + sizeof(int)*2);
    if (!a)
        return NULL;
    int* tab = (int*) a->args;
    tab[0] = n;
