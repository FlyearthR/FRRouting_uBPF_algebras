...
import ubpf_michel.assembler
import ubpf_michel.disassembler
import testdata
...
def try_disassemble(inst):
    ...
    try:
        return ubpf_michel.disassembler.disassemble(data).strip()
    except ValueError:
        return "<error>"
...
def check_datafile(filename):
    ...
    assembled = ubpf_michel.assembler.assemble(data['asm'])
    disassembled = ubpf_michel.disassembler.disassemble(assembled)
    reassembled = ubpf_michel.assembler.assemble(disassembled)
    disassembled2 = ubpf_michel.disassembler.disassemble(reassembled)
